...
namespace CGSLibrary
{
    public class Gallery
    {
        ...
        public bool WriteCurators()
        {
            ...
            //gets the folder of the exe file usualy in the folder \bin\Debug
            string path = System.IO.Directory.GetCurrentDirectory();
            string file = path + "\\curators.txt";

            StreamWriter curatorsFile = new StreamWriter(file);
            
            foreach (Curator c in curators)
            {
                
                curatorsFile.Write(c.FullName + "," + c.CuratorID + "," + c.Commission);
                if (curators.IndexOf(c) != curators.Count - 1)
                {
                    curatorsFile.WriteLine();
                }
            }
            curatorsFile.Close();
            return true;
        ...
        public bool ReadCurators()
        {
            ...
            if (File.Exists(file))
            {
                StreamReader curatorsFile = new StreamReader(file);
                string line;
                string[] prop;
                while ((line = curatorsFile.ReadLine()) != null)
                {
                    prop = line.Split(',');
                    Curator curator = new Curator(prop[0], prop[1], double.Parse(prop[2]));
                    if (curators == null)
                        curators = new List<Curator>();
